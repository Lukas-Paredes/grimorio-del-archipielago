# Proceso de Ancud (1880) — Declaraciones de los reos
...
## Política de transcripción (regla de fidelidad)

Esta transcripción es **verbatim**. No se inventa, completa, parafrasea ni "mejora" nada.

- **Ortografía de época preservada íntegra.** No se moderniza: «i» por «y» (*i*, *hai*, *mui*), «j» por g/x (*jente*, *oríjen*, *dijo*, *imajinacion*), monosílabos con tilde de época (*fué*, *dió*, *vió*, *á*, *ó*, *é*), *espediente*, *espuso*, *estravagancia*, *rejistro*, *asociacion*/*institucion* tal como aparecen.
- **Solo se corrige basura inequívoca de escaneo**, cuando la palabra correcta es indudable contra el original: sustituciones de letra (*tue*→*fue*, *Rncud*→*Ancud*, *Coiiuecar*→*Coñuecar*, *engallar*→*engañar*), espacios intrusos dentro de palabra (*se te nta*→*setenta*) y guiones de corte de línea.
- **La acentuación del crudo se respeta tal cual** (incluida su inconsistencia de época); solo se restauran tildes en nombres propios inequívocos y en un puñado de palabras de altísima frecuencia donde la ausencia es claramente daño de escaneo.
- **Toda palabra dudosa o ilegible** se marca `[ilegible]` o `[¿palabra?]`. Jamás se rellena con una suposición.
- El desespaciado mecánico y los guiones de corte no se listan uno a uno. Las correcciones sustantivas (nivel palabra) y los `[ilegible]`/`[¿?]` se registran en el **Aparato crítico** al final, para QA contra el PDF.

El topónimo aparece en la fuente sin tilde (*Tenaun*); se conserva así (forma moderna: *Tenaún*).

---

## A. Marco del proceso

> **Advertencia (aparato editorial, no de la fuente).** El mandato pedía aquí «la orden de recogida (intendente Martiniano Rodríguez)». **Esa orden NO aparece en esta fuente:** el folleto de Ponce Hermanos (1908) contiene el marco narrativo editorial, las declaraciones de los reos y la sentencia, pero no una orden de la Intendencia. Lo único que la fuente aporta como marco procesal es que el proceso lo levantó el **juez letrado de Ancud** en febrero de 1880. La orden del intendente, de existir, proviene de otra pieza de la cadena de custodia y requiere una fuente aparte (ver `pendientes.md`).
>
> El folleto abre además con una narración editorial de 1908 de tono colonial y racista. **No se transcribe aquí** como «marco» (queda íntegra en el OCR crudo de `_raw/`); ver la NOTA curatorial al inicio de este documento sobre por qué en el sitio se presentan las voces y no ese marco.
...
FUENTE:: Ponce Hermanos, *Los brujos de Chiloé. Célebre proceso del Juzgado de Ancud*, Santiago, 1908 (marco editorial, pp. iniciales). Memoria Chilena, Biblioteca Nacional de Chile, MC0033459 — <https://www.memoriachilena.gob.cl/archivos2/pdfs/MC0033459.pdf>
...
FUENTE:: Ponce Hermanos, *Los brujos de Chiloé. Célebre proceso del Juzgado de Ancud*, Santiago, 1908 (declaración de Mateo Coñuecar, 26 de Marzo de 1880). Memoria Chilena, Biblioteca Nacional de Chile, MC0033459 — <https://www.memoriachilena.gob.cl/archivos2/pdfs/MC0033459.pdf>
...
FUENTE:: Ponce Hermanos, *Los brujos de Chiloé. Célebre proceso del Juzgado de Ancud*, Santiago, 1908 (declaración de Aurora Quinchem Agüil, 27 de Marzo de 1880). Memoria Chilena, Biblioteca Nacional de Chile, MC0033459 — <https://www.memoriachilena.gob.cl/archivos2/pdfs/MC0033459.pdf>
...
FUENTE:: Ponce Hermanos, *Los brujos de Chiloé. Célebre proceso del Juzgado de Ancud*, Santiago, 1908 (sentencia del juez Beytía, 2 de Marzo de 1881, y cierre editorial). Memoria Chilena, Biblioteca Nacional de Chile, MC0033459 — <https://www.memoriachilena.gob.cl/archivos2/pdfs/MC0033459.pdf>

---

## Aparato crítico (para QA — no es la fuente)

Sección de trabajo. **No forma parte del texto verbatim**; existe solo para que Lucas coteje contra el PDF. El OCR crudo íntegro está en [`_raw/proceso-ancud-1880_OCR-crudo.txt`](_raw/proceso-ancud-1880_OCR-crudo.txt).

### Correcciones sustantivas de OCR (crudo → limpio)

Solo se listan las correcciones de nivel palabra (sustitución de letras, nombres reconstruidos). **No** se listan el desespaciado mecánico (*se te nta*→*setenta*) ni los guiones de corte de línea, que se aplicaron en todo el texto.

| # | Crudo (OCR) | Limpio | Bloque | Criterio |
|---|-------------|--------|--------|----------|
| 1 | `Rncud` | Ancud | portada/A | R→A, inequívoco (nombre del juzgado repetido) |
| 2 | `PON· CE HERMANOS` | Ponce Hermanos | portada/A | punto intruso |
| 3 | `IMPRENTA 1 CASA EDITORA` | Imprenta i Casa Editora | portada/A | «1»→«i» (=y), ortografía de época |
| 4 | `tue llevada` | fue llevada | A | t→f, ejemplo canónico del mandato |
| 5 | `Reda Prouidencio` | Recta Providencia | A | ejemplo canónico del mandato |
| 6 | `engallar` | engañar | A | ll→ñ, inequívoco |
| 7 | `se valla n` | se valían | A | ll→lí, inequívoco |
| 8 | `hombres temibles i so mbr ic s` | …i [¿sombríos?] | A | **marcado como dudoso**, no confirmado |
| 9 | `Afaleo Coiiuecar` | Mateo Coñuecar | B, D | «Afaleo»→«Mateo» (confirmado por bloque B); «Coiiuecar»→«Coñuecar» |
| 10 | `Ce rnandame de la Tierra` | «[¿Comandante?] de la Tierra» | B | **marcado**; rol «Comandante» confirmado en jerarquía, pero grafía mui dañada |
| 11 | `no sa be leer L escribir` | no sabe leer i escribir | B | L→i |
| 12 | `el alijen de esa asociacion` | el oríjen… | B | «alijen»→«oríjen», por contexto |
| 13 | `dominacion espa-1'\01a` | dominacion española | B | reconstrucción de «española» |
| 14 | `l\1oraleda` / `Mor aled a` | Moraleda | B | grafía OCR de «Moraleda» |
| 15 | `Cueva de (}Ni` / `Quicaví` (fragmento) | «Cueva de Quicaví» | B | fragmento de página; «Quicaví» confirmado en la línea siguiente |
| 16 | `cl bwu hi` / `l bunche` / `Ibunche` | Ibunche | B | grafías OCR del mismo término |
| 17 | `Picochilzuim` / `p icochihuin` | Picochihuín | B | unificado a la grafía legible |
| 18 | `Benito Nancuaote` (l.267) | Benito Nancuante | B | la fuente usa ambas; se unifica a «Nancuante» (l.229) |
| 19 | `Miguel Rai cagüin` | Miguel Raicagüin | B | desespaciado de nombre |
| 20 | `Jase Maria Chiguai` | José Maria Chiguai | B | «Jase»→«José» |
| 21 | `Fabian Cárdenas` / hijos `Fra ncisco i José Maria` | Fabian / Francisco / José Maria Cárdenas | B | desespaciado |
| 22 | `Pedro Lebltureo` | Pedro [¿Lebiureo?] | B | **marcado**, apellido incierto |
| 23 | `Justo Pe ran de Cala` | Justo [¿Peran?] de Cala | B | **marcado**, apellido incierto |
| 24 | `Eusebio Pinda en Tuevigüe` | …en [¿Tuevigüe?] | B | **marcado**, topónimo incierto |
| 25 | `Cristieo Qu;,uhem` | Cristino Quinchem | D | por contexto (Cristina/Cristino Quinchem en decl.) |
| 26 | `Doming o Coiiuecar` | Domingo Coñuecar | D | desespaciado + ñ |
| 27 | `R ecia Prorincin` / `R ecia Prooincia` | Recta Provincia | C, D | grafía OCR |
| 28 | `Sa fl chez` / `SalUhtz` | Sánchez | B, C | firma del actuario |
| 29 | `Goi, olea` / `Goicolea` | Goicolea | B, C, D | firma del secretario |
| 30 | `el cal-deni·l/o` (hueñoto) | el [¿cardenillo?] | C | **marcado**, término dudoso |
| 31 | `Copuca 1I1i1lahui,dlúl` | Copuca [¿millahuinhuin?] | C | **marcado**, término mui dañado |
| 32 | `Gultin` / `El cachin` | **[¿Cuchin?]** (título) / cachin | C | **marcado**; el título de párrafo y el cuerpo discrepan en el OCR |
| 33 | yerbas `Ibircu'l`, `AIe.fiieIIÚI`, `ióircun`, `ibercun` | [¿Ibircun?], [¿Alefuein?], [¿ibercun?] | C | **marcadas**, fitónimos ilegibles |
| 34 | `Espilla de 1IIec!Jai` / `ma!Jltl` | Espina de [¿mechai?] | C | **marcado** |
| 35 | `Pn..ú, d'l` / `la pUlllada` | [¿Puñalada?] (título) | C | **marcado**; cuerpo dice «la puñalada» |
| 36 | `Reparador de Hayar e` | «Reparador de [¿Payos?]» | C | **marcado**, lugar incierto |
| 37 | `el cúmulo de maldades i alme nes` | …i [¿crímenes?] | D | **marcado**; «crímenes» por contexto, grafía dañada |
| 38 | `sus lu,hiurias` (sentencia) | sus [¿brujerías?] | D | **marcado**, por contexto |

### Lista de `[ilegible]` / `[¿palabra?]` (dudas marcadas, nunca rellenadas)

Reproducidas para tu QA rápido. Todas quedaron marcadas en el texto; ninguna se rellenó con suposición.

- **Bloque A:** `[¿sombríos?]` (hombres temibles i…).
- **Bloque B:**
  - `«[¿Comandante?] de la Tierra»` (título del hermano Andrés Coñuecar).
  - `[¿en caso de faltar a alguna?] de estas promesas` (fórmula del juramento; centro del pasaje mui dañado en el OCR).
  - `Eusebio Pinda en [¿Tuevigüe?]` (topónimo).
  - `Pedro [¿Lebiureo?]` (apellido).
  - `Justo [¿Peran?] de Cala` (apellido).
- **Bloque C:**
  - `el [¿cardenillo?]` (voz «hueñoto»).
  - `Copuca [¿millahuinhuin?]`.
  - `[¿Cuchin?]` / `cachin` (título del párrafo vs. cuerpo).
  - yerbas `[¿Ibircun?]`, `[¿Alefuein?]`, `[¿ibercun?]`.
  - `Espina de [¿mechai?]`.
  - `[¿Puñalada?]` (título del remedio).
  - `«Reparador de [¿Payos?]»` (título de José Parancan).
- **Bloque D:**
  - `sus [¿brujerías?]` (donde cometen…).
  - `el cúmulo de maldades i [¿crímenes?]`.

### Notas de fidelidad

- **Nombres con variantes en la propia fuente:** *Quinchem/Quinchen*, *Quinchepane/Quinchipane*, *Nancuante/Nancuaote*, *Merimañ* (varias grafías), *Cristina/Cristino Quinchem*. Se conservó una forma coherente por persona y se dejó constancia aquí; ninguna es invención.
- **Topónimo *Tenaun*:** la fuente lo escribe sin tilde; se conserva (forma moderna: *Tenaún*).
- **La declaración de Coñuecar** incluye un largo relato de oríjen (Moraleda / Chillpila / cueva de Quicaví / Imbunche i Chibato) que se transcribe completo por ser central al expediente; no se abrevió.
- Las declaraciones previas del expediente (Santiago Rain Güichaman i Cipriano Rain, sobre el homicidio de Andrés Netor, marzo–mayo de 1879) **no** forman parte de este dossier de la Recta Provincia; quedan en el OCR crudo de `_raw/` por si se necesitan más adelante.
