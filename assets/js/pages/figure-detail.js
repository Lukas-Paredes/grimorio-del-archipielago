(function () {
  ...
  function relationshipList(figure) {
    ...
    return '<ul class="relationship-list">' + relations.map(function (rel) {
      ...
      var href = otherFigure ? "figura.html?id=" + encodeURIComponent(other) : "mundo.html?id=" + encodeURIComponent(other);
  ...
  G.renderCurrentPage = function () {
    ...
    if (!figure) {
      ...
      root.innerHTML = '<section class="not-found"><span class="eyebrow">Archivo</span><h1>Ficha no encontrada</h1><p>El identificador de la URL no corresponde a una de las 24 fichas disponibles.</p><a class="button" href="bestiario.html">Volver al Bestiario</a></section>';
      return;
    }
    var world = G.getWorld(figure.worldId);
    var returnUrl = params.get("return") || "bestiario.html";
    if (/^(?:[a-z]+:)?\/\//i.test(returnUrl) || returnUrl.indexOf("..") !== -1) {
      returnUrl = "bestiario.html";
    }
    document.title = figure.name + " · Bestiario del Archipiélago";
    root.innerHTML = [
      '<nav class="breadcrumbs" aria-label="Migas de pan"><a href="index.html">Inicio</a><span aria-hidden="true">/</span><a href="bestiario.html">Bestiario</a><span aria-hidden="true">/</span><span aria-current="page">', G.escape(figure.name), '</span></nav>',
      ...
      '<a class="button button--quiet" href="mundo.html?id=', encodeURIComponent(figure.worldId), '">Explorar su mundo</a></div>',
      ...
      '<footer class="dossier-footer"><a class="button" href="mundo.html?id=', encodeURIComponent(figure.worldId), '">Abrir Mundo ', G.escape(world ? world.number : ""), '</a><a class="text-link" href="metodologia.html">Consultar metodología y estados</a></footer>',
      '</article>'
    ].join("");
  };
}());
