(function () {
  ...
  G.figureCard = function (figure, options) {
    ...
    return [
      ...
      '<div class="figure-card__meta"><span>', G.escape(world ? "Mundo " + world.number + " · " + world.shortTitle : "Archivo"), '</span><span class="figure-card__status">', G.escape(G.sealLabel(figure.status)), '</span></div>',
  ...
  G.figureRow = function (figure) {
    ...
    return '<li class="figure-row"><a href="' + G.pageUrl("figura.html", "?id=" + encodeURIComponent(figure.id)) + '"><strong>' +
      ...
      '<small class="figure-card__status">' + G.escape(G.sealLabel(figure.status)) + "</small></li>";
  };
}());
