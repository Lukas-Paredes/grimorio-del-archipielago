# Pendientes — backlog de El Grimorio del Archipiélago

_Generado por `herramientas/generar_fichas.py`. Es la lista de trabajo:
qué imágenes faltan por criatura, si hay prosa curada y si está publicada._

> Las decisiones y notas escritas a mano NO van en este archivo: este se
> regenera completo. Van en `docs/DECISIONES-PENDIENTES.md`.

Slots de imagen: `hero` (16:9) · `lamina` (2:3, retrato) · `descenso` (9:16) · `cierre` (16:9) · `card` (1:1).

| id | sección | imágenes (`<id>-<slot>`) | prosa | publicada |
|---|---|---|---|---|
| tenten-caicai | 01-origen | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| millalobo | 02-corte-del-mar | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| huenchula | 02-corte-del-mar | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| huenchur | 02-corte-del-mar | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| pincoya | 02-corte-del-mar | hero:sí lamina:— descenso:sí cierre:sí card:— | sí | sí |
| pincoy | 02-corte-del-mar | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| sirena-chilota | 02-corte-del-mar | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| caleuche | 03-barcos-y-almas | hero:sí lamina:— descenso:— cierre:— card:— | sí | sí |
| tempilcahue | 03-barcos-y-almas | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| animas-de-cucao | 03-barcos-y-almas | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| trauco | 04-bosque | hero:sí lamina:— descenso:sí cierre:sí card:— | sí | sí |
| fiura | 04-bosque | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| ruende | 04-bosque | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| trehuaco | 04-bosque | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| camahueto | 05-bestias | hero:sí lamina:— descenso:sí cierre:sí card:— | sí | sí |
| cuchivilu | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| caballo-marino | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| cuero | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| vaca-marina | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| nirivilu | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| cochodoma | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| sumpall | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| lucerna | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| basilisco | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| piuchen | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| cahuelche | 05-bestias | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| coñipoñi | 06-menores | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| coñieuma | 06-menores | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| vilpoñi | 06-menores | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| piruquina | 06-menores | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| den | 06-menores | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| lluhay | 06-menores | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| puyo | 06-menores | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| carbunco | 06-menores | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| curamilla | 06-menores | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| coo | 07-aves-brujas | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| raiquen | 07-aves-brujas | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| mandao | 07-aves-brujas | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| chonchon | 07-aves-brujas | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| llorona-pucullen | 08-penas | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| viuda | 08-penas | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| condena | 08-penas | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| caballero-de-lata | 08-penas | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| brujo-chilote | 09-recta-provincia | hero:— lamina:sí descenso:— cierre:— card:— | sí | sí |
| machi | 09-recta-provincia | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| calcu | 09-recta-provincia | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| recta-provincia | 09-recta-provincia | hero:sí lamina:— descenso:— cierre:— card:— | sí | sí |
| la-mayoria | 09-recta-provincia | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| siete-republicas | 09-recta-provincia | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| iniciacion | 09-recta-provincia | hero:— lamina:sí descenso:— cierre:— card:— | sí | — |
| macun | 09-recta-provincia | hero:— lamina:sí descenso:— cierre:— card:— | sí | sí |
| poderes-del-brujo | 09-recta-provincia | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| challanco | 09-recta-provincia | hero:— lamina:sí descenso:— cierre:— card:— | sí | sí |
| invunche | 09-recta-provincia | hero:sí lamina:— descenso:sí cierre:sí card:— | sí | sí |
| voladora | 09-recta-provincia | hero:— lamina:sí descenso:— cierre:— card:— | sí | sí |
| cueva-quicavi | 09-recta-provincia | hero:— lamina:sí descenso:— cierre:— card:— | sí | sí |
| libro-de-moraleda | 09-recta-provincia | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| juicio-1880 | 09-recta-provincia | hero:sí lamina:— descenso:— cierre:— card:— | — | sí |
| muelle-de-las-almas | 10-lugares | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| cucao | 10-lugares | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| quicavi | 10-lugares | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| tenaun | 10-lugares | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| achao | 10-lugares | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| entierros | 11-relatos | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| campana-sumergida | 11-relatos | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| islita-imelev | 11-relatos | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| origen-de-la-papa | 11-relatos | hero:— lamina:— descenso:— cierre:— card:— | — | — |
| animitas | 11-relatos | hero:— lamina:— descenso:— cierre:— card:— | — | — |

## Resumen
- Entidades: **68**
- Publicadas: **12**
- Con prosa curada: **12**
- Con hero: **7**

## Pendiente de imágenes (Gemini)
- `portada-invunche` y `portada-camahueto` (fondo de umbral de sus capítulos)
  **por generar en Gemini**; hoy usan su propio hero con los velos del sistema
  (provisional coherente, decisión del mandato 2026-07-05).
- `fiura-hero/descenso/cierre` LLEGARON en la cosecha 2026-07-05 (no se esperaban):
  respaldadas en `assets/img/_raw/`, **sin procesar ni montar** hasta decisión
  (la ficha necesita además prosa curada y entrada en el camino).
- `camahueto-cierre` vino a mitad de resolución (1376×768, no 2752×1536):
  montada tal cual (sin inventar píxeles); regenerar a resolución completa si se puede.
- Siguen por generar: sigilos, ornamentos, separadores, botones, marco, niebla-capas.

## Frentes del proyecto
- **Postulación al fondo: ENVIADA / CERRADA — frente congelado.** No regenerar
  PDFs ni tocar `postulacion/`. El mockup quedó con la sección 11 (equipo de tres)
  y datos reales de portada. Prioridad única: la plataforma (audio, pulido).
- **Deploy a GitHub Pages**: parqueado — requiere hacer el repo PÚBLICO (decisión
  de Lucas; el plan gratuito no sirve Pages en repos privados). URL prevista:
  `lukas-paredes.github.io/grimorio-del-archipielago` (ya cableada en canonical/og/sitemap).
- **Mapa del archipiélago (mandato 2026-07-13)**: el acceso ya está en la
  botonera (panel placeholder). ANTES de montar la capa geográfica real,
  **verificar los puntos ⚠️** de `fuentes/investigacion/geografia-cosmologia.md`
  (cascadas Tocoihue y Nalhuitad, cueva de Colo, playa de Quicaví/brujo
  Zapata — candidatos: Quintana 1972 / Cavada 1914). Los 8 puntos ✅ ya son
  mapeables con fuente en repo.

## Recta Provincia — dossier del proceso de Ancud (1880)
- **Fuente primaria domesticada: LISTA.** `fuentes/proceso-ancud-1880.md`
  (transcripción verbatim: marco procesal, declaración de Mateo Coñuecar,
  declaración de Aurora Quinchem y sentencia del juez Beytía, 1881) + el OCR
  crudo y el PDF fuente en `fuentes/_raw/` (Ponce Hermanos, 1908; Memoria
  Chilena MC0033459).
- **Montaje EN CURSO (mandato 2026-07-10, decisiones cerradas).** Ambos
  capítulos bajo `libro-8`. Fichas plenas: brujo-chilote, recta-provincia,
  cueva-quicavi, macun, challanco, voladora (+ invunche ya publicado) y
  juicio-1880. Vitrina (sin parada propia): machi, calcu, la-mayoria,
  siete-republicas, poderes-del-brujo, libro-de-moraleda, iniciacion.
  Doble columna mito‖testimonio vía `@TESTIMONIO` (flag aditivo);
  «El Expediente» = página bespoke `juicio-1880.html`; slot `lamina` 2:3;
  `recogida.png` FUERA (ilustra la orden del intendente, sin fuente 1908).
- **CAPÍTULO COMPLETO MONTADO (piloto aprobado 2026-07-10):** paradas V-XI
  (cueva-quicavi, recta-provincia, brujo-chilote, macun, challanco,
  voladora, juicio-1880) + vitrina de 7 piezas en el hub recta-provincia +
  «El Expediente» (juicio-1880.html, bespoke, en la lista BESPOKE del
  generador). Pendiente: QA de Lucas del capítulo completo; su cotejo de
  las marcas [¿?] contra el PDF sigue pendiente antes de publicar a
  producción; `n` definitivo espera CAMINO-DEL-MITO.
- **Vacío de fuente (no inventar):** la «orden de recogida» del intendente
  Martiniano Rodríguez NO aparece en el folleto de 1908. El dossier transcribe
  el marco procesal que sí existe (el juez letrado de Ancud levantó el proceso,
  feb. 1880) y marca el vacío. Si Lucas tiene esa pieza (Intendencia /
  Vicuña Mackenna / Espech), pasarla para completar el Bloque A.
- **QA pendiente (tu ojo):** cotejar el «Aparato crítico» del .md contra el
  PDF — ~13 marcas `[¿?]` (topónimos, fitónimos y apellidos ilegibles) y la
  tabla de correcciones crudo→limpio.
- **Prensa de época (2026-07-11):** registro rastreable en
  `fuentes/_raw/prensa/REGISTRO-PRENSA.md`. Pistas fuertes SIN descargar:
  el decreto del intendente (7-abr-1880) publicado en El Chilote (8-abr) y
  El Liberal (9-abr) — completaría el Bloque A—; la sentencia de 1ª
  instancia en la Gaceta de los Tribunales (11-jun-1881), digitalizada como
  serie en la BND (visor BND:620043, descarga manual pendiente). «El Correo
  7-ago-1880» NO verificó contra Catepillan: pista por confirmar.

## Decisiones y compras de Lucas (mandato 2026-07)
- **Documento NotebookLM (CAMINO DEL MITO)**: pendiente de entrega. El orden
  vigente (mandato 2026-07-05) es Caleuche → Pincoya → Trauco → Invunche →
  Camahueto; al llegar el documento, ajustar `n` y cargar los
  `puente { texto, fuente }` en `assets/js/data/capitulos.js` (todo data-driven).
- **Dominio + hosting**: seguir `DEPLOY.md` (NIC Chile para el .cl + GitHub Pages).
  Al confirmar dominio: renombrar `CNAME.placeholder` → `CNAME` y revisar
  canonical/og/sitemap si el dominio difiere del placeholder.
- **Scroll-snap**: probar en Android real; si marea, quitar la clase `snap-on` de <html>.

